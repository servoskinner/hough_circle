...
def area_traverse(array, x0, y0, markup=None, leap_size=1):
    ...
    step_options = np.array(
        [
            [hor, ver]
            for hor in range(-leap_size, leap_size + 1)
            for ver in range(-leap_size, leap_size + 1)
        ]
    )
    # traverse the grid
    if markup[x0, y0] != 0:
    ...
    while len(neighbours) > 0:
        ...
        for option in options:
            ...
            if (
                np.all(option >= 0)
                and np.all(option < array.shape)
                and markup[tuple(option)] == 0
                and array[tuple(option)] == area_class
            ):
                # add to queues
    ...
    for x in range(array.shape[0]):
        for y in range(array.shape[1]):
            if array_norm[x, y] != 0 and traversal_progress[x, y] == 0:
                ...
                cluster_height, cluster_width, center = area_traverse(
                    array_norm, x, y, markup=traversal_progress, leap_size=leap_size
                )
