...
def get_circle_kernel(radius, point_density = 2, npoints = None, intensity = None):
    ...
    draw_circle(kernel, k_size/2, k_size/2, radius, point_density=point_density,
                                                    npoints=npoints,
                                                    intensity=intensity)
    return kernel
...
def shrink_image(image, factor):
    ...
    for x in range(image.shape[0]):
        for y in range(image.shape[1]):
            draw_subpixel(image_shrunk, 0.5 + x/factor, 0.5 + y/factor, image[x, y]/factor**2)
    return image_shrunk

def hough_circle(source_img, radius, point_density = 2.0, shrink_factor = 1.0, conv_mode = 'same'):
